...
bool submit(const Observation& o) {
    if(!obs_queue || xQueueSend(obs_queue,&o,0)!=pdTRUE) {critical_fault=true;return false;}
    return true;
}
...
static void allocation_failed(size_t,uint32_t,const char*) {critical_fault=true;}
static void setup(void*) {
    ...
    if(r==ReadResult::Ok && valid_profile(profile) && !critical_fault) {
        ...
        start_wifi(profile);
        if(!start_management(profile))critical_fault=true;
        start_tesla(profile);
    } else if(r==ReadResult::Ok)critical_fault=true;
    vTaskDelete(nullptr);
}
}
extern "C" void app_main() {
    ...
    if(!board::initialize_off())critical_fault=true;
    if(!esp_psram_is_initialized() || esp_psram_get_size()<8*1024*1024)critical_fault=true;
    vTaskPrioritySet(nullptr,10);
    if(heap_caps_register_failed_alloc_callback(allocation_failed)!=ESP_OK)critical_fault=true;
    if(esp_task_wdt_add(nullptr)!=ESP_OK)critical_fault=true;
    obs_queue=xQueueCreateStatic(8,sizeof(Observation),obs_queue_buffer,&obs_queue_data);
    if(!obs_queue || xTaskCreate(setup,"setup",12288,nullptr,2,nullptr)!=pdPASS)critical_fault=true;
    ...
    while(true) {
        Ms now=now_ms();
        if(now-last_tick>250)critical_fault=true;
        last_tick=now;
        ...
        if(!board::command(d.commanded)) {critical_fault=true;board::command(false);d.commanded=false;}
        ...
        if(esp_task_wdt_reset()!=ESP_OK)critical_fault=true;
